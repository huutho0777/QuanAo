...
if (isset($_POST['themgiohang'])) {
	...
	$sanpham_id = isset($_POST['sanpham_id']) ? $_POST['sanpham_id'] : '';
	$hinhanh = isset($_POST['hinhanh']) ? $_POST['hinhanh'] : '';
	$gia = isset($_POST['giasanpham']) ? $_POST['giasanpham'] : 0;
	$soluong = isset($_POST['soluong']) ? $_POST['soluong'] : 1;

	// Kiểm tra nếu sanpham_id không hợp lệ
	if (empty($sanpham_id)) {
		die("Lỗi: Mã sản phẩm không hợp lệ.");
	}
	...
	if ($sql_khachhang) {
		...
		$mahang = rand(0, 9999); //ma khi dat hang
		...
		for ($i = 0; $i < count($_POST['thanhtoan_product_id']); $i++) {
			...
			$sql_donhang = mysqli_query($con, "INSERT INTO tbl_donhang(sanpham_id, khachhang_id, soluong, mahang, tinhtrang)
                                               VALUES ('$sanpham_id', '$khachhang_id', '$soluong', '$mahang', 0)");
			$sql_giaodich = mysqli_query($con, "INSERT INTO tbl_giaodich(sanpham_id, soluong, magiaodich, khachhang_id, tinhtrangdon)
						...
						<tbody>
							...
							?>
								<tr class="rem1">
									...
									<td class="invert"><?php echo number_format($row_fetch_giohang['giasanpham']) . 'vnđ' ?>
									</td>
